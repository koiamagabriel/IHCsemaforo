...
|---|---|---|---|---|---|---|---|
| H01 | Pesquisadores ou analistas de mobilidade representam adequadamente o usuário prioritário da interface. | H | O perfil escolhido influencia tarefas, linguagem, contexto, personas e decisões de interação. | Entregas 2, 3 e 7 | Entrega 2 — A análise do Aimsun Next e do PTV Vissim identificou ferramentas profissionais que materializam atividades de configuração, execução e análise de simulações de transporte semelhantes às previstas para o projeto. Entrega 3 — P01 foi estruturada como proto-persona primária, consolidando objetivos, necessidades, dores e contexto de uso compatíveis com esse perfil. Essas evidências reforçam a plausibilidade do usuário priorizado, mas ainda não representam validação direta com usuários reais. | aberta | As Entregas 2 e 3 reforçam a plausibilidade do perfil escolhido, mas H01 permanece aberta e deverá ser investigada diretamente com usuários ou especialistas na Entrega 7. |
| H02 | Organizar a interação em definição da malha, configuração/execução e análise dos resultados facilita a realização dos experimentos em relação à interação direta com as ferramentas técnicas. | H | Esse fluxo constitui a estrutura inicial do projeto de interface e precisa ser validado. | Entregas 3, 4, 5, 6 e 7 | Entrega 3 — A jornada de P01 organizou o fluxo entre definição do estudo, preparação do cenário, configuração, execução e análise. Entrega 4 — C01 aprofundou as rupturas relacionadas à preparação e execução de experimentos sob condições comparáveis. Esses artefatos estruturam a hipótese, mas não constituem validação direta com usuários. | aberta | H02 permanece aberta. C01 indica quais decisões, informações e rupturas deverão ser aprofundadas na análise de tarefas, prototipação e posterior avaliação com usuários. |
| H03 | Uma apresentação comparativa das métricas por meio de dashboard ou relatório facilita a interpretação do desempenho dos métodos. | H | A forma de apresentação influencia a compreensão e comparação dos resultados produzidos pelos experimentos. | Entregas 2, 4, 6, 7 e 12–14 | Entrega 2 — Aimsun Next e PTV Vissim utilizam resumos, listas, séries de dados e gráficos para apresentar e comparar resultados de simulações. Entrega 3 — P01 e P02 indicaram necessidades relacionadas à interpretação e comparação dos resultados. Entrega 4 — C02 aprofundou as dificuldades de contextualizar métricas, verificar comparabilidade e construir uma síntese dos resultados. Essas evidências ainda não confirmam qual formato de apresentação será mais adequado ao usuário. | aberta | H03 permanece aberta para validação por prototipação e avaliação com usuários. C02 reforça a necessidade de investigar como apresentar contexto, métricas e comparações sem antecipar uma solução específica. |
---
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R01 | Utilização de diferentes malhas e cenários no ambiente experimental. | Definir corretamente onde e em quais condições o experimento será realizado. | P01 | C01 | T01 — definir ou selecionar a malha viária. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R02 | Configuração dos parâmetros utilizados nos experimentos. | Definir corretamente as condições da simulação antes da execução. | P01 | C01 | T02 — configurar parâmetros do experimento. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R03 | Execução de diferentes estratégias de controle semafórico. | Selecionar claramente qual estratégia será utilizada em cada experimento. | P01 | C01 | T03 — selecionar a estratégia de controle. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R04 | Execução automatizada do ambiente experimental. | Realizar o experimento configurado e compreender seu estado de execução. | P01 | C01 | T04 — iniciar uma simulação; T05 — verificar o estado da execução. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R05 | Produção de métricas de desempenho das simulações. | Compreender o desempenho obtido em cada experimento. | P01, P02 | C02 | T06 — consultar as métricas obtidas. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R06 | Comparação experimental entre tempo fixo, Max Pressure e Max Pressure + actuated. | Comparar o comportamento das estratégias sob condições equivalentes. | P01, P02 | C02 | T07 — comparar resultados. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R07 | Geração e organização dos resultados experimentais. | Registrar e comunicar os resultados obtidos em uma execução ou comparação. | P01, P02 | C02 | T08 — consultar ou gerar uma síntese dos resultados. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
...
| PENDENTE | Acompanhamento de processamento | T04 e T05 | Iniciar a simulação e compreender seu estado de execução. | Entrega 1 + Entrega 2: controles de execução e feedback de estado observados no Aimsun Next e no PTV Vissim; RC02 | R04 |
| PENDENTE | Dashboard/visão de resultados | T06 e T07 | Visualizar métricas e comparar o desempenho dos experimentos. | H03 + Entrega 2: Aimsun Next e PTV Vissim utilizam resumos, listas e visualizações gráficas de resultados; RC06, RC07 e RC09 | R05, R06 |
| PENDENTE | Relatório/resultados | T06, T07 e T08 | Consultar, sintetizar e comunicar os resultados obtidos. | H03 + Entrega 2: resultados vinculados às execuções e diferentes formas de visualização observadas no Aimsun Next e no PTV Vissim; RC05, RC06 e RC07 | R05, R06, R07 |
