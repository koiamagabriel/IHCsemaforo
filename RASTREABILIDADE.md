...
| H03 | Uma apresentação comparativa das métricas por meio de dashboard ou relatório facilita a interpretação do desempenho dos métodos. | H | A forma de apresentação influencia a compreensão e comparação dos resultados produzidos pelos experimentos. | Entregas 2, 6, 7 e 12–14 | Entrega 2 — Aimsun Next e PTV Vissim utilizam resumos, listas, séries de dados e gráficos para apresentar e comparar resultados de simulações. Isso fornece evidência de que visualizações consolidadas e comparativas são padrões presentes em ferramentas do domínio, mas ainda não confirma qual formato será mais adequado ao nosso usuário. | aberta | A Entrega 2 reforça a relevância de investigar visualizações consolidadas e comparativas; H03 permanece aberta para validação por prototipação e avaliação com usuários. |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R01 | Utilização de diferentes malhas e cenários no ambiente experimental. | Definir corretamente onde e em quais condições o experimento será realizado. | P01 | PENDENTE | T01 — definir ou selecionar a malha viária. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R02 | Configuração dos parâmetros utilizados nos experimentos. | Definir corretamente as condições da simulação antes da execução. | P01 | PENDENTE | T02 — configurar parâmetros do experimento. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R03 | Execução de diferentes estratégias de controle semafórico. | Selecionar claramente qual estratégia será utilizada em cada experimento. | P01 | PENDENTE | T03 — selecionar a estratégia de controle. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R04 | Execução automatizada do ambiente experimental. | Realizar o experimento configurado e compreender seu estado de execução. | P01 | PENDENTE | T04 — iniciar uma simulação; T05 — verificar o estado da execução. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R05 | Produção de métricas de desempenho das simulações. | Compreender o desempenho obtido em cada experimento. | P01, P02 | PENDENTE | T06 — consultar as métricas obtidas. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R06 | Comparação experimental entre tempo fixo, Max Pressure e Max Pressure + actuated. | Comparar o comportamento das estratégias sob condições equivalentes. | P01, P02 | PENDENTE | T07 — comparar resultados. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
| R07 | Geração e organização dos resultados experimentais. | Registrar e comunicar os resultados obtidos em uma execução ou comparação. | P01, P02 | PENDENTE | T08 — consultar ou gerar uma síntese dos resultados. | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE | PENDENTE |
...
| PENDENTE | Acompanhamento de processamento | T04 e T05 | Iniciar a simulação e compreender seu estado de execução. | Entrega 1 + Entrega 2: controles de execução e feedback de estado observados em C01 Aimsun Next e C02 PTV Vissim; RC02 | R04 |
| PENDENTE | Dashboard/visão de resultados | T06 e T07 | Visualizar métricas e comparar o desempenho dos experimentos. | H03 + Entrega 2: C01 e C02 utilizam resumos, listas e visualizações gráficas de resultados; RC06, RC07 e RC09 | R05, R06 |
| PENDENTE | Relatório/resultados | T06, T07 e T08 | Consultar, sintetizar e comunicar os resultados obtidos. | H03 + Entrega 2: resultados vinculados às execuções e diferentes formas de visualização observadas em C01/C02; RC05, RC06 e RC07 | R05, R06, R07 |
